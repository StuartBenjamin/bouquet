...
versus calling ``generate_perturbed_GPR`` (which does a fresh eigh on
every call).
"""
...
class TestGPRTiming:
    ...
    def test_timing_comparison(self, grid, capsys):
        ...
        perturber.precompute_factor(psi_N, sigma)   # O(n³) once
        ...
        with capsys.disabled():
            ...
            print(f"{'='*55}")
            print(f"  Re-draw  (precompute_factor + draw_from_factor):")
            print(f"    total : {t_redraw*1e3:.1f} ms")
            print(f"    per   : {t_redraw/N_DRAWS*1e3:.3f} ms/draw")
    ...
    def test_timing_batch_vs_loop(self, grid, capsys):
        """Batch draw (n_samples > 1) vs loop of single draws."""
        psi_N, profile, sigma = grid

        perturber = GPRProfilePerturber(kernel_func="rbf", length_scale=LENGTH)
        perturber.precompute_factor(psi_N, sigma)
        ...
        t0 = time.perf_counter()
        for _ in range(N_DRAWS):
            perturber.draw_from_factor(profile, 1, rng_b)
        ...
        with capsys.disabled():
            ...
            print(f"{'='*55}")
            print(f"  Batch draw_from_factor(n={N_DRAWS}): {t_batch*1e3:.2f} ms")
            print(f"  Loop  draw_from_factor(n=1)×{N_DRAWS}: {t_loop*1e3:.2f} ms")
            print(f"  Ratio batch/loop: {t_batch/t_loop:.2f}")
            print(f"{'='*55}")

    def test_timing_grid_scaling(self, capsys):
        """Show how timing scales with grid size for both methods."""
...
class TestVerifyGPRStatistics:
    ...
    The test passes only if the two paths agree to within 5 % (the
    assertion is inside ``verify_gpr_statistics`` itself).
    """
    ...
    def test_verify_prints_and_agrees(self, capsys):
        ...
        for key in ("redraw", "batch"):
            exc = result[key]["avg_exceedance"]
            assert abs(exc - theory) < 0.02, (
